...
function DarkroomTask({ task, todo }: { task: Task; todo: ReturnType<typeof useTodos> }) {
...
export default function DarkroomVariant() {
  const todo = useTodos();
  ...
  return (
    <div className="relative h-screen w-screen overflow-hidden bg-[#241414]">
      ...
      <div className="absolute inset-0 z-10 flex items-center justify-center">
        {media.loading ? (
          <p className="text-sm text-red-900/50">Developing...</p>
        ) : (
            ...
            <>
              ...
              <ul>
                {todo.visibleTasks.map((task) => (
                  <DarkroomTask key={task.id} task={task} todo={todo} />
                ))}
              </ul>
            </>
          )}
      ...
      <VariantNav
        current={3}
        className="absolute left-5 top-5 z-30 flex items-center gap-1"
        pillClassName="flex h-6 w-6 items-center justify-center rounded text-[10px] transition-all"
        activeClassName="bg-red-900/30 text-red-400"
        inactiveClassName="text-red-950/30 hover:text-red-700/50"
      />
    </div>
  );
}
